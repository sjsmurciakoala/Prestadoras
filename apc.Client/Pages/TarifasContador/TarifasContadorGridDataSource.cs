...
namespace apc.Client.Pages.TarifasContador;

public sealed class TarifasContadorGridDataSource : GridCustomDataSource
{
    private readonly TarifasContadorClient _client;
    private readonly Action? _onStateChanged;

    public TarifasContadorGridDataSource(TarifasContadorClient client, Action? onStateChanged = null)
    {
        _client = client;
    ...
    public override async Task<int> GetItemCountAsync(GridCustomDataSourceCountOptions options, CancellationToken cancellationToken)
    {
        ...
        {
            SetState(null, $"No se pudo cargar tarifas de contador: {ex.Message}");
            return 0;
        }
    }
    ...
    public override async Task<IList> GetItemsAsync(GridCustomDataSourceItemsOptions options, CancellationToken cancellationToken)
    {
        try
        {
            ...
            return result?.Items?.ToList() ?? new List<TarifaContadorListItemDto>();
        }
        catch (OperationCanceledException)
        {
            return new List<TarifaContadorListItemDto>();
        }
        catch
        {
            SetState(null, "No se pudo cargar tarifas de contador.");
            return new List<TarifaContadorListItemDto>();
        }
    }

    private TarifaContadorFilterDto BuildFilter() => new()
    {
    ...
    private int SetState(PagedResult<TarifaContadorListItemDto>? result, string? error)
    {
