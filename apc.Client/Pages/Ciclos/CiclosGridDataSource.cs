...
namespace apc.Client.Pages.Ciclos;

public sealed class CiclosGridDataSource : GridCustomDataSource
{
    private readonly CiclosClient _client;
    private readonly Action? _onStateChanged;

    public CiclosGridDataSource(CiclosClient client, Action? onStateChanged = null)
    {
        _client = client;
    ...
    public override async Task<int> GetItemCountAsync(GridCustomDataSourceCountOptions options, CancellationToken cancellationToken)
    {
        ...
        {
            SetState(null, $"No se pudo cargar el catalogo de ciclos: {ex.Message}");
            return 0;
        }
    }
    ...
    public override async Task<IList> GetItemsAsync(GridCustomDataSourceItemsOptions options, CancellationToken cancellationToken)
    {
        try
        {
            ...
            return result?.Items?.ToList() ?? new List<CicloListItemDto>();
        }
        catch (OperationCanceledException)
        {
            return new List<CicloListItemDto>();
        }
        catch
        {
            SetState(null, "No se pudo cargar el catalogo de ciclos.");
            return new List<CicloListItemDto>();
        }
    }

    private CicloFilterDto BuildFilter() => new()
    {
        Search = string.IsNullOrWhiteSpace(SearchText) ? null : SearchText,
        Activo = SoloActivos ? true : null
    };

    private int SetState(PagedResult<CicloListItemDto>? result, string? error)
    {
