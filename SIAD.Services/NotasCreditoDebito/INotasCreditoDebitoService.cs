...
public interface INotasCreditoDebitoService
{
    ...
    Task<NotaImpresionDto?> ObtenerNotaImpresionAsync(string tipoNota, long notaId, CancellationToken ct = default);
    Task<(NotaImpresionDto? Nota, string? Error)> GenerarVistaPreviaCreditoAsync(EmitirNotaCreditoRequestDto dto, CancellationToken ct = default);
    Task<(NotaImpresionDto? Nota, string? Error)> GenerarVistaPreviaDebitoAsync(EmitirNotaDebitoRequestDto dto, CancellationToken ct = default);
