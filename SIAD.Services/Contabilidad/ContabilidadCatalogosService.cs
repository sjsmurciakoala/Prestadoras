...
public class ContabilidadCatalogosService : IContabilidadCatalogosService
{
    ...
    public async Task<IReadOnlyList<DocumentTypeLookupDto>> GetDocumentTypesAsync(string? module, CancellationToken cancellationToken = default)
    {
        var companyId = EnsureCompanyId();
        var normalizedModule = NormalizeSimpleCode(module);

        var query = _context.cfg_document_types
            .AsNoTracking()
            .Where(d => d.company_id == companyId);

        if (!string.IsNullOrWhiteSpace(normalizedModule))
        {
            query = query.Where(d => d.module == normalizedModule);
        }

        return await query
            .OrderBy(d => d.module)
            .ThenBy(d => d.code)
            .Select(d => new DocumentTypeLookupDto(
                d.document_type_id,
                d.module,
                d.code,
                d.name,
    ...
    public async Task<IReadOnlyList<ReglaIntegracionDto>> GetReglasIntegracionAsync(ReglaIntegracionFilterDto filter, CancellationToken cancellationToken = default)
    {
        var companyId = EnsureCompanyId();
        var normalizedModule = NormalizeSimpleCode(filter.Module);
        var search = filter.Search?.Trim();

        var query = _context.con_regla_integracions
            .AsNoTracking()
        ...
        {
            query = query.Where(r => r.document_type_id == filter.DocumentTypeId.Value);
        }
        ...
        if (!string.IsNullOrWhiteSpace(search))
        {
            ...
            query = query.Where(r =>
                r.scenario_code.ToUpper().Contains(term) ||
                (r.description != null && r.description.ToUpper().Contains(term)));
        }

        var documentTypes = _context.cfg_document_types
            .AsNoTracking()
            .Where(d => d.company_id == companyId);

        var accounts = _context.con_plan_cuentas
            .AsNoTracking()
            .Where(a => a.company_id == companyId);

        var costCenters = _context.con_centro_costos
            .AsNoTracking()
            .Where(c => c.company_id == companyId);

        var result = await (from r in query
                            join d in documentTypes on r.document_type_id equals d.document_type_id
                            join debit in accounts on r.debit_account_id equals debit.account_id
                            join credit in accounts on r.credit_account_id equals credit.account_id
                            join cc in costCenters on r.cost_center_id equals cc.cost_center_id into ccJoin
                            from cc in ccJoin.DefaultIfEmpty()
                            orderby r.module, d.code, r.scenario_code
                            select new ReglaIntegracionDto(
                                r.regla_id,
                                r.company_id,
                                r.module,
                                d.document_type_id,
    ...
    public async Task<long> SaveReglaIntegracionAsync(ReglaIntegracionUpsertDto request, CancellationToken cancellationToken = default)
    {
        var companyId = EnsureCompanyId();
        var normalizedModule = NormalizeSimpleCode(request.Module);
        var normalizedScenario = NormalizeSimpleCode(request.ScenarioCode);

        if (string.IsNullOrWhiteSpace(normalizedModule))
        {
            throw new ArgumentException("El módulo es obligatorio.", nameof(request));
        }

        if (string.IsNullOrWhiteSpace(normalizedScenario))
        {
            throw new ArgumentException("El escenario es obligatorio.", nameof(request));
        }

        var documentType = await _context.cfg_document_types
            .AsNoTracking()
            .FirstOrDefaultAsync(d => d.company_id == companyId && d.document_type_id == request.DocumentTypeId, cancellationToken);
        ...
        if (!string.Equals(documentType.module, normalizedModule, StringComparison.OrdinalIgnoreCase))
        {
            throw new InvalidOperationException("El documento no pertenece al módulo seleccionado.");
        }

        var debitAccount = await _context.con_plan_cuentas
            .AsNoTracking()
            .FirstOrDefaultAsync(a => a.company_id == companyId && a.account_id == request.DebitAccountId, cancellationToken);

        if (debitAccount is null || !debitAccount.allows_posting)
        {
            throw new InvalidOperationException("Cuenta débito no válida.");
        }

        var creditAccount = await _context.con_plan_cuentas
            .AsNoTracking()
            .FirstOrDefaultAsync(a => a.company_id == companyId && a.account_id == request.CreditAccountId, cancellationToken);

        if (creditAccount is null || !creditAccount.allows_posting)
        {
            throw new InvalidOperationException("Cuenta crédito no válida.");
        }
        ...
        if (request.CostCenterId.HasValue)
        {
            var ok = await _context.con_centro_costos
                .AsNoTracking()
                .AnyAsync(c => c.company_id == companyId && c.cost_center_id == request.CostCenterId.Value, cancellationToken);

            if (!ok)
            {
                throw new InvalidOperationException("Centro de costo no válido.");
            }
        }
        ...
        if (request.RuleId.HasValue)
        {
            entity = await _context.con_regla_integracions
                .FirstOrDefaultAsync(r => r.company_id == companyId && r.regla_id == request.RuleId.Value, cancellationToken)
        ...
        var exists = await _context.con_regla_integracions
            .AsNoTracking()
            .AnyAsync(r =>
                r.company_id == companyId &&
                r.module == normalizedModule &&
                r.document_type_id == request.DocumentTypeId &&
                r.scenario_code == normalizedScenario &&
                r.regla_id != entity.regla_id, cancellationToken);

        if (exists)
        {
            throw new InvalidOperationException("Ya existe una regla con este módulo, documento y escenario.");
        }

        entity.module = normalizedModule;
        entity.document_type_id = request.DocumentTypeId;
        entity.scenario_code = normalizedScenario;
        entity.description = request.Description?.Trim();
    ...
    public async Task<bool> DeleteReglaIntegracionAsync(long ruleId, CancellationToken cancellationToken = default)
    {
        var companyId = EnsureCompanyId();
        var entity = await _context.con_regla_integracions
            .FirstOrDefaultAsync(r => r.company_id == companyId && r.regla_id == ruleId, cancellationToken);
