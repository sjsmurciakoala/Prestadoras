-- =============================================================================
-- F7 H5c — rendimiento de los reportes sobre el modelo nuevo
-- =============================================================================
-- Los 9 rep_* que leen vw_rep_movimiento_vigente filtraban por
-- COALESCE(ta.fecha_docu, ta.fecha_registro): una expresión que ningún índice
-- puede servir y que además impide empujar el predicado a las ramas del UNION.
-- En la vista nueva fecha_registro ES fecha_docu en todas las ramas (misma
-- columna proyectada dos veces), así que COALESCE(a,a) = a: se reemplaza por
-- ta.fecha_docu — semántica EXACTA, cero cambio de resultados (verificado por
-- hash de las 9 salidas contra la corrida previa).
--
-- Con el predicado simple, Postgres lo empuja a cada rama del UNION ALL como
-- fechaemision / fecha / fecha_emision, y estos índices lo sirven:
--   * factura (company_id, fechaemision)           — rama de cargos
--   * adm_pago (company_id, fecha)                 — ramas de créditos/remanente
--   * adm_nota_credito / _debito (company_id, fecha_emision)
--
-- Nota: los términos de "saldo anterior" (< fecha_desde) siguen recorriendo la
-- historia acumulada — eso es volumen, no plan. En el disco USB local siguen
-- siendo minutos; en un disco de servidor, segundos.
-- =============================================================================
...
CREATE INDEX IF NOT EXISTS ix_factura_company_fechaemision
    ON public.factura (company_id, fechaemision);

CREATE INDEX IF NOT EXISTS ix_adm_pago_company_fecha
    ON public.adm_pago (company_id, fecha);

CREATE INDEX IF NOT EXISTS ix_adm_nota_credito_company_fecha
    ON public.adm_nota_credito (company_id, fecha_emision);

CREATE INDEX IF NOT EXISTS ix_adm_nota_debito_company_fecha
    ON public.adm_nota_debito (company_id, fecha_emision);

-- ---------------------------------------------------------------------------
-- La vista se re-emite idéntica a H5b (rama 3 con agregado por hash). Se
-- probó la variante LATERAL para el remanente: mejora las consultas de
-- ventana corta pero DUPLICA el costo de los reportes de saldo (historia
-- completa = 2.8M sondas de índice). Medido el 30-jul: categoria_cobranza
-- 11:13→26:40 con LATERAL. Se descarta; el agregado por hash es el mejor
-- equilibrio para el mix real de reportes.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Las 9 funciones, regeneradas desde el catálogo con el único cambio:
-- COALESCE(ta.fecha_docu, ta.fecha_registro) -> ta.fecha_docu
-- ---------------------------------------------------------------------------
...
ultimo_saldo_cliente AS (
    ...
    WHERE ta.company_id = p.company_id
      ...
      AND ta.fecha_docu < p.fecha_desde
...
pagos_raw AS (
    SELECT
        ta.fecha_docu AS fecha_movimiento,
...
ANALYZE public.adm_nota_credito;
ANALYZE public.adm_nota_debito;
