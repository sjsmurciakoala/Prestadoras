-- =============================================================================
-- Fix multitenancy — el motor de factura usaba el saldo SIN filtrar company_id
-- Fecha: 2026-07-07
-- Rama: feat/snapshot-campos-piloto
-- Construye sobre el PR #12 (merge baacbda, fix-resolver-medidos): NO cambia el
-- cálculo tarifario, solo migra la fuente del saldo previo a la firma company-scoped.
--
-- Defecto (regla #1 del repo — toda tabla funcional se filtra por company_id):
--   sp_adm_calcular_factura_lectura resolvía el saldo previo del cliente con el
--   overload de 1 argumento public.sp_obtener_cliente_saldo(pcodigocliente), que
--   filtra SOLO por cliente_clave + estado='A' (WHERE ta.cliente_clave = ... ) y
--   NO por company_id. En multi-empresa, dos empresas pueden tener claves de
--   cliente colisionantes: el overload de 1-arg toma el ÚLTIMO movimiento activo
--   por ide GLOBAL, así que podía devolver el saldo de OTRA empresa. Como el saldo
--   previo entra en `saldos_anteriores`, en el recargo por mora (ROUND(saldo*tasa))
--   y en `total_factura`, el motor podía facturar el saldo del cliente equivocado.
--
--   Hoy es LATENTE (prod tiene una sola empresa: company_id=2), pero hay que
--   cerrarlo antes de incorporar la 2ª empresa. El snapshot offline ya usaba la
--   firma correcta (2-arg) desde 2026-05-09; el motor online quedó rezagado
--   (ver el follow-up anotado en el PR #13 / mora-en-snapshot).
--
-- Fix: migrar la llamada al overload company-scoped
--   public.sp_obtener_cliente_saldo(p_company_id, v_clave)  (firma de 2026-05-09,
--   20260509_sp_obtener_cliente_saldo_company.sql). p_company_id ya está en el
--   scope de la función. Es el ÚNICO cambio respecto a la definición del PR #12;
--   el resto del cuerpo se preserva byte-a-byte.
--
-- Alcance: solo el motor V3 sp_adm_calcular_factura_lectura. Los callers legacy
--   del 1-arg (sp_lectura, sp_lectura_v2, sp_registrar_posteo_manual — flujo Java
--   viejo en retiro por L12) NO tienen company_id en su firma y quedan fuera de
--   alcance a propósito; el overload de 1-arg sigue vivo (DEPRECATED) para ellos.
--
-- Idempotente: CREATE OR REPLACE, firma 100% preservada. Solo cambia el body
-- (una línea de la fuente del saldo).
-- =============================================================================
