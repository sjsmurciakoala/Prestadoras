-- =============================================================================
-- Snapshot offline: campos que el ticket fiscal y la factura necesitan (piloto)
-- Fecha: 2026-07-07
-- Rama: feat/snapshot-campos-piloto
-- Construye sobre el PR #12 (tramos CM) y el PR #13 (bloque mora): NO reemplaza
-- el snapshot, lo EXTIENDE. Documentado por L9/L10 de la app de lectores: el
-- snapshot no llevaba datos que la app imprime offline (ticket + factura).
--
-- Se agregan tres cosas al snapshot_json, todas MULTITENANT (A6, por p_company_id):
--   1. "emisor": encabezado de la empresa que imprime el ticket (razón social,
--      RTN del emisor, dirección, nombre comercial, teléfono, email). Fuente:
--      public.cfg_company filtrado por p_company_id. Es EXACTAMENTE la misma
--      fuente y prioridad de nombre (legal_name → commercial_name → code) que usa
--      el encabezado de los reportes del portal (SIAD.Reports/
--      ReportCompanyHeaderParameters.cs), para que el ticket offline coincida.
--   2. "cliente_rtn": RTN del abonado (cliente_maestro.maestro_cliente_rtn), que
--      la factura imprime como identificación fiscal del comprador. Ya viene en
--      v_cliente (cargado company-scoped); no requiere consulta extra.
--   3. "fecha_lectura_anterior": fecha de la lectura anterior del medidor
--      (historicomedicion.fecha_lect_ant). Es el mismo campo que el path online
--      del lector expone como FechaLecturaAnterior (LectoresMobileService). Va
--      NULL cuando no hay histórico del período (igual que lect_ant → 0).
--
-- Multitenancy (además del A6 de los campos nuevos): la carga de v_historico se
-- filtra ahora por company_id. Antes seleccionaba SOLO por ano/mes/clave; con
-- claves colisionantes entre empresas podía tomar el histórico de otra empresa.
-- El filtro protege los campos ya existentes (lectura_anterior_referencia,
-- promedio_referencia) y el nuevo fecha_lectura_anterior. En single-company el
-- resultado es idéntico (regresión preservada).
--
-- contract_version se mantiene en OFFLINE_SNAPSHOT_V3_2 (cambio aditivo, mismo
-- precedente que #12 y #13): la app detecta los campos nuevos por PRESENCIA. No
-- se toca el const C# MobileApiDtos.SnapshotContractVersion ni los consumidores
-- (el snapshot_json viaja como string opaco dentro de PackageJson).
--
...
DECLARE
    ...
    -- FIX 2026-07-07 (snapshot-campos-piloto): encabezado del emisor para el ticket
    -- fiscal. Fuente cfg_company filtrada por p_company_id (multitenant A6).
    v_emisor_legal_name text;
    v_emisor_commercial_name text;
    v_emisor_code text;
    v_emisor_tax_id text;
    v_emisor_address text;
    v_emisor_phone text;
    v_emisor_email text;
...
BEGIN
    ...
    WHERE hm.company_id = p_company_id  -- FIX 2026-07-07: A6, evita histórico cross-company por clave colisionante
    ...
    -- ── Encabezado del emisor (ticket fiscal + factura), multitenant por p_company_id ──
    -- Misma fuente y prioridad de nombre que el encabezado de reportes del portal
    -- (SIAD.Reports/ReportCompanyHeaderParameters.cs): legal_name → commercial_name → code.
    SELECT c.legal_name, c.commercial_name, c.code, c.tax_id, c.address, c.phone, c.email
      INTO v_emisor_legal_name, v_emisor_commercial_name, v_emisor_code,
           v_emisor_tax_id, v_emisor_address, v_emisor_phone, v_emisor_email
    ...
    SELECT
        ...
        jsonb_build_object(
            ...
            -- FIX 2026-07-07 (snapshot-campos-piloto): encabezado del emisor que el
            -- ticket fiscal imprime (razón social, RTN emisor, dirección). Multitenant:
            -- cfg_company de p_company_id. 'nombre' con la misma prioridad que el portal.
            'emisor', jsonb_build_object(
                'nombre', COALESCE(
                    NULLIF(BTRIM(COALESCE(v_emisor_legal_name, '')), ''),
                    NULLIF(BTRIM(COALESCE(v_emisor_commercial_name, '')), ''),
                    NULLIF(BTRIM(COALESCE(v_emisor_code, '')), ''),
                    'EMPRESA'
                ),
                'nombre_comercial', NULLIF(BTRIM(COALESCE(v_emisor_commercial_name, '')), ''),
                'rtn', NULLIF(BTRIM(COALESCE(v_emisor_tax_id, '')), ''),
                'direccion', NULLIF(BTRIM(COALESCE(v_emisor_address, '')), ''),
                'telefono', NULLIF(BTRIM(COALESCE(v_emisor_phone, '')), ''),
                'email', NULLIF(BTRIM(COALESCE(v_emisor_email, '')), '')
            ),
            ...
            -- FIX 2026-07-07 (snapshot-campos-piloto): RTN del abonado (comprador) que
            -- la factura imprime. Ya viene en v_cliente (cargado company-scoped).
            'cliente_rtn', NULLIF(BTRIM(COALESCE(v_cliente.maestro_cliente_rtn, '')), ''),
            ...
            -- FIX 2026-07-07 (snapshot-campos-piloto): fecha de la lectura anterior del
            -- medidor (historicomedicion.fecha_lect_ant). NULL si no hay histórico del
            -- período. Mismo dato que el path online expone como FechaLecturaAnterior.
            'fecha_lectura_anterior', v_historico.fecha_lect_ant,
