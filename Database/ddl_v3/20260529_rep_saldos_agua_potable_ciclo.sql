-- =============================================================================
-- Reporte: Saldos de agua potable por ciclo
-- Origen para el dataset web public.rep_saldos_agua_potable_ciclo
-- =============================================================================

DROP FUNCTION IF EXISTS public.rep_saldos_agua_potable_ciclo(bigint, date, date);
DROP FUNCTION IF EXISTS public.rep_saldos_agua_potable_ciclo(bigint, date, date, bigint);
DROP FUNCTION IF EXISTS public.rep_saldos_agua_potable_ciclo(bigint, date, date, integer);

CREATE OR REPLACE FUNCTION public.rep_saldos_agua_potable_ciclo(
    p_company_id bigint,
    p_fecha_desde date,
    p_fecha_hasta date,
    p_ciclo_id bigint DEFAULT 0
)
RETURNS TABLE (
    ciclo_orden integer,
    ciclo text,
...
ultimo_saldo_anterior AS (
    SELECT DISTINCT ON (ta.cliente_clave)
        ...
        COALESCE(ta.saldo_detalle, 0)::numeric(18, 2) AS saldo_anterior
...
COMMENT ON FUNCTION public.rep_saldos_agua_potable_ciclo(bigint, date, date, bigint) IS
'Resumen totalizado por ciclo de saldos de agua potable únicamente, con saldo anterior, movimientos del periodo y conteos de clientes. Soporta filtrado por un ciclo específico.';
