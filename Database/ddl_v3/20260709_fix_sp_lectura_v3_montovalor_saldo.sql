-- =============================================================================
-- 2026-07-09  Fix sp_lectura_v3: factura_detalle.montovalor_saldo
-- -----------------------------------------------------------------------------
-- BUG: al emitir la factura, la línea de detalle guardaba `montovalor_saldo`
-- con `v_saldo_servicio_actual` (el saldo del servicio ANTES de sumar el cargo
-- del período; 0 en una factura nueva), en vez de `v_saldo_detalle` (saldo del
-- servicio YA incluido el cargo). Como `fn_ban_ws_pendientes` cobra leyendo
-- COALESCE(montovalor_saldo, montovalor, 0) > 0, las facturas nacían en 0 y el
-- WS bancario respondía "No hay pagos pendientes" — no cobrables.
--
-- FIX: en el INSERT de factura_detalle, `montovalor_saldo := v_saldo_detalle`.
-- Solo cambia ese campo de cobro; el total de la factura, el estado de cuenta
-- (transaccion_abonado.saldo, que ya usaba v_saldo_detalle) y la partida
-- contable NO se tocan. Es CREATE OR REPLACE de la definición viva completa.
--
-- APLICAR TAMBIÉN EN PROD (172.16.0.9 siad_v3). Datos ya emitidos se corrigen
-- con: UPDATE factura_detalle SET montovalor_saldo = montovalor
--      WHERE COALESCE(montovalor_saldo,0)=0 AND montovalor>0;  (según corresponda)
-- =============================================================================
