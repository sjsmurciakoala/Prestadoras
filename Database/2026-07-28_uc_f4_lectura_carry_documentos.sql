...
-- sp_lectura_v3: el arrastre de saldo por servicio (montovalor_saldo de las
-- líneas nuevas) se CAPTURA de los documentos pendientes ANTES de compensar
-- la factura anterior. Motivo: en F4 sp_obtener_cliente_saldo_servicio_detalle
-- pasó a leer líneas de facturas A/B; como la compensación (estado 'C') corre
-- antes del loop de detalles, la llamada dentro del loop devolvería 0 y el
-- arrastre se perdería. La captura previa reproduce el valor correcto y de
-- paso corrige dos bugs del camino viejo:
--   1. La corrida legacy saldo_detalle quedaba DESACTUALIZADA para clientes
--      con pagos del motor (arrastraba de más lo ya pagado).
--   2. El UPDATE de compensación no filtraba company_id (cross-company) y la
--      función por servicio se llamaba con el overload sin company.
-- La lógica de cálculo (sp_adm_calcular_factura_lectura) queda intacta.
-- Base: definición vigente de 20260714_calendario_facturacion.sql.
-- ============================================================================
