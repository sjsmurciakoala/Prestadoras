...
Write-Host ""
Write-Host "Ingresa el nombre para el archivo de backup (deja en blanco para usar el nombre por defecto 'backup_antes_renombrar_columnas_$timestamp.backup'):" -ForegroundColor Yellow -NoNewline
$customFileName = Read-Host

if ([string]::IsNullOrWhiteSpace($customFileName)) {
    $backupFileName = "backup_antes_renombrar_columnas_$timestamp.backup"
} else {
    $backupFileName = $customFileName
    if ($backupFileName -notlike "*.backup") {
