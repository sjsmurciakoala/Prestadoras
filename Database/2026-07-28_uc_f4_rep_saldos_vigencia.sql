-- ============================================================================
-- Unificacion de cobranza - F4 (2026-07-28)
-- Reportes rep_*: una sola regla de saldo/pagos.
--
-- Correcciones (los numeros CAMBIAN a proposito — antes salian mal):
--  * ta.saldo / ta.saldo_detalle (columnas corridas) estaban corruptas para
--    abonos y desactualizadas con los pagos del motor → se recalcula: saldos
--    por cliente desde la vista de vigencia; saldos por servicio desde los
--    DOCUMENTOS pendientes (lineas de facturas A/B).
--  * Los bloques de "pagos" filtraban estado 'A' + tipo '201': en la
--    convencion de caja 'A' es ANULADO (los vigentes van con 'C'), no
--    contaban los pagos del banco (202) ni los PAGO% migrados.
--  * rep_saldo_clientes_categoria ahora incluye el residuo migrado de SIMAFI
--    en el bucket OTROS (sus filas llevan saldo_detalle = 0 y se omitian).
--
-- El cambio de fuente completo a adm_pago/documentos para rangos HISTORICOS
-- llega en F7 con la re-migracion (la historia pre-motor solo existe en
-- transaccion_abonado). rep_listado_corte_masivo no se toca: lee el snapshot
-- de cln_corte_masivo_dtl.
-- Generado desde las definiciones vigentes (pg_get_functiondef) +
-- transform_rep.py; los cuerpos no comentados son identicos a los vigentes.
-- ============================================================================
