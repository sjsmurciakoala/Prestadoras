...
AS $BODY$
DECLARE
    -- Codigo de ruta normalizado a 5 digitos (estandar V3) para evitar matches
    -- ambiguos como '01'::int = '0001'::int = 1.
...
BEGIN
    ...
    LEFT JOIN (
        ...
        where cm2.estado = true
          and (
            CASE WHEN split_part(cm2.maestro_cliente_indicativo_ruta, '-', 3) ~ '^[0-9]+$'
                 THEN lpad(split_part(cm2.maestro_cliente_indicativo_ruta, '-', 3), 5, '0')
                 ELSE split_part(cm2.maestro_cliente_indicativo_ruta, '-', 3)
            END = v_ruta_norm
            or cm2.maestro_cliente_indicativo_ruta = v_ruta_norm
          )
    ...
    LEFT JOIN (
        ...
        where cm3.estado = true
          and (
            split_part(cm3.maestro_cliente_indicativo_ruta, '-', 3) = v_ruta_norm
            or cm3.maestro_cliente_indicativo_ruta = v_ruta_norm
          )
    ...
    WHERE cm.estado = true
      AND (
        split_part(cm.maestro_cliente_indicativo_ruta, '-', 3) = v_ruta_norm
