...
public sealed class ServiciosController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<IActionResult> Get([FromQuery] ServicioFilterDto filtro, CancellationToken ct)
    {
        var servicios = await _service.GetAsync(filtro, ct);
        return Ok(servicios);
    }

    [HttpGet("paged")]
    public async Task<IActionResult> GetPaged(
        [FromQuery] ServicioFilterDto filtro,
        [FromQuery] int skip,
        [FromQuery] int take,
    ...
    {
        var servicio = await _service.GetByIdAsync(id, ct);
        return servicio is null ? NotFound() : Ok(servicio);
    }

    [HttpPost]
    public async Task<IActionResult> Create([FromBody] ServicioEditDto dto, CancellationToken ct)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromBody] ServicioEditDto dto, CancellationToken ct)
    {
        if (!ModelState.IsValid)
        {
            return ValidationProblem(ModelState);
