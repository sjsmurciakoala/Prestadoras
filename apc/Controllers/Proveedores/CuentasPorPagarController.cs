...
/// exigiendo el rol de Contabilidad, igual que su propia pantalla. Por eso el lote comprueba el
/// rol solo cuando lleva compromisos dentro.
/// </para>
/// </summary>
[ApiController]
...
public sealed class CuentasPorPagarController : ControllerBase
{
    ...
    public CuentasPorPagarController(ICuentasPorPagarService service) => _service = service;

    private string Usuario => User?.Identity?.Name ?? "system";

    private bool PuedePagarCompromisos =>
        User.IsInRole(RoleNames.SuperAdministrador) ||
        User.IsInRole(RoleNames.Admin) ||
        User.IsInRole(RoleNames.Contabilidad);
    ...
    public async Task<IActionResult> PagarLote([FromBody] CxpLoteUpsertDto dto, CancellationToken ct)
    {
        ...
        if (llevaCompromisos && !PuedePagarCompromisos)
        {
            return Problem(
