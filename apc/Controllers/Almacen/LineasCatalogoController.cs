...
[ApiController]
[Route("api/almacen/lineas")]
[ModuleAuthorize(PermissionModules.Inventario)]
public sealed class LineasCatalogoController : ControllerBase
{
    private readonly ILineaService _service;
    public LineasCatalogoController(ILineaService service) => _service = service;
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] LineaEditDto dto, CancellationToken ct)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromBody] LineaEditDto dto, CancellationToken ct)
    {
        if (!ModelState.IsValid) return ValidationProblem(ModelState);
