...
[ApiController]
[Route("api/almacen/terminos-pago")]
[ModuleAuthorize(PermissionModules.Inventario)]
public sealed class TerminosPagoController : ControllerBase
{
    private readonly ITerminoPagoService _service;
    public TerminosPagoController(ITerminoPagoService service) => _service = service;
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] TerminoPagoEditDto dto, CancellationToken ct)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromBody] TerminoPagoEditDto dto, CancellationToken ct)
    {
        if (!ModelState.IsValid) return ValidationProblem(ModelState);
        try
        {
