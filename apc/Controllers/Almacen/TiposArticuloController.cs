...
[ApiController]
[Route("api/almacen/tipos-articulo")]
[ModuleAuthorize(PermissionModules.Inventario)]
public sealed class TiposArticuloController : ControllerBase
{
    private readonly ITipoArticuloService _service;
    public TiposArticuloController(ITipoArticuloService service) => _service = service;
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] TipoArticuloEditDto dto, CancellationToken ct)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromBody] TipoArticuloEditDto dto, CancellationToken ct)
    {
        if (!ModelState.IsValid) return ValidationProblem(ModelState);
