...
public sealed class GruposController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] GrupoEditDto dto, CancellationToken ct)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromBody] GrupoEditDto dto, CancellationToken ct)
    {
        if (!ModelState.IsValid) return ValidationProblem(ModelState);
